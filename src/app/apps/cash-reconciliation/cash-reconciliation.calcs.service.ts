...
export class CashReconciliationCalcsService {
  calculateCumulativeTotals(
    ...
    const difference: number = currency(totalAmount).subtract(cashJournalAmount).value;
    let outcome: string = '';
    if (difference < 0) {
      outcome = 'Short';
  ...
  setDeposit(cashDesk: Partial<CashDesk>, cashJournalAmount: number, selectedShift: 'Opening' | 'Closing'){
    const cumulativeTotals: {totalAmount: number, difference: number, outcome: string} = this.calculateCumulativeTotals(cashDesk, cashJournalAmount, selectedShift);
    const eodEarnings: number = cumulativeTotals.totalAmount;
    let depositAmount: number = 0;
    let depositBreakdown: {cashDenomination: number, countOfBillCoin: number, cashValue: number}[] = [];
    for(const [key, value] of Object.entries(cashDesk)){
      let count: number = value.count;
      if(isNil(value.count)){
        count = 0;
      }
      let interimAmount: number = currency(depositAmount).add(value.multiplier).multiply(count).value;
      while(interimAmount > eodEarnings && count > 0){
        count -= 1;
        interimAmount = currency(depositAmount).add(value.multiplier).multiply(count).value;
      }
